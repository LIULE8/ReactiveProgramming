...
public class ReactiveAccountClient {
  ...
  @Autowired private AccountRedisRepository accountRedisRepository;
  ...
  private Mono<AccountMapper> putAccountIntoCache(AccountMapper account) {
    ...
    return accountRedisRepository
        ...
        .flatMap(
            saved -> {
              Mono<AccountMapper> savedAccount =
                  accountRedisRepository.findAccountById(account.getId());
  ...
  public Mono<AccountMapper> findAccountById(String accountId) {
    ...
    Mono<AccountMapper> accountMono =
        accountMonoFromCache.switchIfEmpty(getAccountFromRemote(accountId));
