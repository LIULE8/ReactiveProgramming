...
public class ReactiveAccountChangedSource {
  ...
  public ReactiveAccountChangedSource() {
    this.flux =
        Flux.<Message<AccountChangedEvent>>create(sink -> this.eventSink = sink)
            .publish()
            .autoConnect();
  }
  ...
  private Mono<Void> publishAccountChangedEvent(String operation, Account account) {
    ...
    AccountMessage accountMessage =
        new AccountMessage(account.getId(), account.getAccountCode(), account.getAccountName());

    AccountChangedEvent originalevent =
        new AccountChangedEvent(AccountChangedEvent.class.getTypeName(), operation, accountMessage);
